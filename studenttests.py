...
class TestAbsLoss(unittest.TestCase):
    def test_simple(self):
        ...
        correct_array = [0, 4, 2, 2, 4, 0, 6, 2, 8]
        t.check_array(array2, correct_array)
...
class TestSquaredLoss(unittest.TestCase):
    def test_simple(self):
        ...
        t.call("squared_loss")
        # check that the result array contains the correct output
        correct_array = [0, 16, 4, 4, 16, 0, 36, 4, 64]
        t.check_array(array2, correct_array)
        # check that the register a0 contains the correct output
    ...
    # Add other test cases if neccesary
    def test_squared_loss_error(self):
        t = AssemblyTest(self, "../coverage-src/squared_loss.s")
...
class TestZeroOneLoss(unittest.TestCase):
    def test_simple(self):
        ...
        # create input arrays in the data section
        array0 = t.array([0, 4, 2, 2, 4, 0, 6, 2, 8])
        array1 = t.array([0, 16, 4, 4, 16, 0, 36, 4, 64])
        # load array addresses into argument registers
        t.input_array("a0", array0)
        t.input_array("a1", array1)
        # load array length into argument register
...
class TestInitializeZero(unittest.TestCase):
    def test_simple(self):
        ...
        correct_array = [0, 0, 0, 0, 0, 0]
        t.check_array_pointer("a0", correct_array)
        t.execute()

    # Add other test cases if neccesary
    def test_initialize_zero_error(self):
        t = AssemblyTest(self, "../coverage-src/initialize_zero.s")
    ...
    def test_initialize_zero_fail_malloc(self):
        t = AssemblyTest(self, "../coverage-src/initialize_zero.s")
        test_dir = "../test-coverage-src"
        ...
        t.call("initialize_zero")
        t.check_scalar("a0", 2)
        t.execute(fail="malloc", code=26, args=args)
